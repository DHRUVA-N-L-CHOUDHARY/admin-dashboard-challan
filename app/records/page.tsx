...
interface RecordModalProps {
  ...
  markAsNotPaid: (recordID: string) => void;
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-96">
        ...
        {!record.isPaid ? (
          <button
            onClick={() => markAsPaid(record._id)} // <-- This should be markAsPaid
          ...
          <button
            onClick={() => markAsNotPaid(record._id)} // <-- This should be markAsPaid
        ...
        <button
          onClick={() => deleteRecord(record._id)} // <-- Add delete functionality
          className="w-full mb-4 px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors"
        >
          Delete Record
        </button>
...
export default function RecordsPage() {
  ...
  useEffect(() => {
    fetchRecords();
  }, [searchTerm, statusFilter, paymentFilter, sortKey, currentPage]);
  ...
  const markAsPaid = async (recordID: string) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/record/paid`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const markAsNotPaid = async (recordID: string) => {
    console.log(recordID);
    try {
      const response = await fetch(`${API_URL}/api/v1/record/notPaid`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteRecord = async (recordID: string) => {
    console.log(recordID);
    try {
      const response = await fetch(`${API_URL}/api/v1/record/delete`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <div className="min-h-screen bg-gray-100 p-8">
      ...
      <div className="flex items-center justify-between mb-6 space-x-4">
        <input
          ...
          className="px-4 py-2 w-full border text-black border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-black focus:border-black placeholder-gray-400 text-sm"
        />
        ...
        <select
          ...
          className="px-4 py-2 border text-black border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-black focus:border-black text-sm"
        >
